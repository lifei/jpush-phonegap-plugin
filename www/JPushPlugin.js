...
JPushPlugin.prototype.call_native = function (name, args, callback) {
    return cordova.exec(callback, this.errorCallback, 'JPushPlugin', name, args);
};
...
JPushPlugin.prototype.addLocalNotification = function (builderId, content, title, notificationID, broadcastTime, extras) {
    if (device.platform == "Android") {
        var data = [builderId, content, title, notificationID, broadcastTime, extras];
